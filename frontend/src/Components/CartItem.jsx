...
import { productReducer } from "../store/ProductsSlice";
 ...
 const CartItem = ({item,dispatch}) => {
  ...
  return (
    ...
    <img src={`${item.img}`} className="w-32 max-h-32 object-contain" alt="" />
    <div className="flex flex-col w-full">
        <div className="flex justify-between">
            <div>
                ...
                <span className="text-gray-400">{item.stockLeft} stocks left</span>
            </div>
        ...
        <div className="flex gap-4 mt-6 justify-between">
            ...
            <span className="text-xl font-bold font-open">&#8377; {item.price.toLocaleString('en-IN')}</span>
        </div>
    </div>
</div>
  )
